...
class Wizard extends Component {
  ...
  steps = () => {
    const steps = [
      ...
      {
        text:
          "SAI transferred - Your requested SAI is transferred to your wallet"
      }
    ];
    ...
    this.setState(state, () => {
      this.setState(prevState => {
        ...
        if (state.eth.gt(0) && this.props.system.eth.myBalance.lt(state.eth)) {
          state.error =
            "The amount of ETH to be deposited exceeds your current balance.";
          return state;
        } else if (state.skr.gt(0) && state.skr.round(0).lte(toWei(0.005))) {
          state.error = `You are not allowed to deposit a low amount of ETH in a CDP. It needs to be higher than 0.005 PETH (${formatNumber(
        ...
        if (state.eth.gt(0) && state.dai.gt(0)) {
          if (
            ...
            state.error =
              "The amount of SAI you are trying to generate exceeds the current system debt ceiling.";
          } else if (state.dai.gt(state.maxDaiAvail)) {
            state.error =
              "The amount of ETH to be deposited is not enough to draw this amount of SAI.";
          } else {
  ...
  mobileHeaderText = () => {
    return this.state.step === 1 ? "Create CDP" : "Confirm";
  };
    ...
    return checkIsMobile.any ? (
      ...
      <header className="col" style={{ borderBottom: "none" }}>
        <Steps current={this.state.step - 1}>
          <Step
            title="Collateralize &amp; generate SAI"
    ...
    return (
      <div className="wizard-section" style={{ marginBottom: padding }}>
        ...
        {this.state.step === 1 ? (
          ...
          <React.Fragment>{this.confirm(stabilityFee)}</React.Fragment>
        )}
      </div>
    );
  }
